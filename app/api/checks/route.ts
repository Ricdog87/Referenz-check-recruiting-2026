...
export async function POST(req: NextRequest) {
  try {
    ...
    const { candidateId, employerName, employerContact, employerPhone, employerEmail, position, startDate, endDate } =
      body

    if (!candidateId || !employerName?.trim()) {
      return NextResponse.json(
        { error: 'Kandidat und Arbeitgeber sind erforderlich.' },
        { status: 400 }
      )
    }
