...
export default function AgencyWaitlistPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Eintrag fehlgeschlagen.')
      setSubmitted(true)
    } catch (e: any) {
      setError(e.message || 'Eintrag fehlgeschlagen.')
    } finally {
      setLoading(false)
    }
  }
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <div className="pt-2 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                  ...
                  <button type="submit" className="btn-primary" disabled={loading}>
                    {loading ? 'Wird gespeichert…' : <>Frühen Zugang vormerken <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </div>
                {error && (
